...
final class EditorWindowController: NSWindowController, NSWindowDelegate {
    init<V: View>(rootView: V) {
        ...
        win.title = "TinyRecorder — Macro Editor"
    ...
    var body: some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("\(rowCount) events · \(formatDuration(duration))")
                    .font(.system(size: 11))
                    .foregroundColor(.secondary)
            }
            Spacer()

            Toggle("Hide mouse moves", isOn: $hideMouseMoves)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                Group {
                    ...
                    HStack {
                        Button("Trim before", action: trimBefore)
                            .help("Delete every event before the selected one")
                        Button("Trim after", action: trimAfter)
                ...
                Group {
                    ...
                    Text("> 1× makes the macro slower, < 1× faster.")
                        .font(.system(size: 10))
                        .foregroundColor(.secondary)
                    Button("Apply to all", action: applyStretch)
                ...
                Group {
                    ...
                    HStack {
                        Button("− earlier") { shiftSelection(by: -shiftMs / 1000.0) }
                        Button("+ later")   { shiftSelection(by:  shiftMs / 1000.0) }
                    }
                    .disabled(selection.isEmpty)
                }
                ...
                Group {
                    ...
                    if selection.count == 1, let id = selection.first,
                       recorder.events.indices.contains(id) {
                        ...
                        Button("Apply changes", action: applyInspector)
                            .buttonStyle(.borderedProminent)
                            .controlSize(.small)
                    } else if selection.count > 1 {
                        Text("\(selection.count) events selected.")
                            .font(.system(size: 11))
                            .foregroundColor(.secondary)
                        Text("Select one event to edit fields.")
                            .font(.system(size: 10))
                            .foregroundColor(.secondary)
                    } else {
                        Text("No selection.")
                            .font(.system(size: 11))
                            .foregroundColor(.secondary)
                        Text("Click a row in the table to inspect or edit it.")
                            .font(.system(size: 10))
                            .foregroundColor(.secondary)
                    }
                }
...
private struct EditorFooter: View {
    ...
    var body: some View {
        Divider()
        HStack(spacing: 16) {
            Text("\(eventCount) events").foregroundColor(.secondary)
            Text("·").foregroundColor(.secondary.opacity(0.5))
            Text(formatDuration(duration)).foregroundColor(.secondary)
            if selectedCount > 0 {
                Text("·").foregroundColor(.secondary.opacity(0.5))
            ...
            Spacer()
            Text("Edits apply immediately. Save the macro to keep them.")
                .foregroundColor(.secondary)
                .font(.system(size: 10))
        }
        .font(.system(size: 11))
